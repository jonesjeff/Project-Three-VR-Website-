    ...
    var startVec = new THREE.Vector3(1, 1, 1);
    var vectorToScale = new THREE.Vector3(1, 1, 1);
    var tweenStart = 0;
    var vreticle = {
        REVISION: '1'
    };
    ...
    vreticle.Reticle = function(camera) {
        ...
        new_reticle.default_material = function() {
            var material = new THREE.MeshBasicMaterial( { 
              ...
              map: THREE.ImageUtils.loadTexture( 'images/Reticle.png' )
          });  
          return material;
        }
        ...
        var default_geometry = new THREE.PlaneGeometry(side_length, side_length, 1);
        // }
        // if (sphere) {
    ...
    new_reticle.create_reticle = function(camera) {
        ...
        this.reticle_object = this.create_default_object(new THREE.Vector3(0, 0, -.5), true, .02, undefined, true);
        ...
        if (intersects.length > 0) {
            ...
            if (this.reticle_hit_object.gazeable) {
                ...
                if (this.gazing_object != null) {
                    vectorToScale = scaleVec.lerpVectors(startVec, endVec, this.reticle_hit_time - this.gazing_time)
    ...
    new_reticle.detect_gaze = function() {
        ...
        if(grow){    
             if(shrink && vectorToScale.x > 0){ 
                this.reticle_object.scale.x = vectorToScale.x;
                this.reticle_object.scale.y = vectorToScale.y;
                pow = true
            }else if(!shrink && pow && vectorToScale.x < 0){
                new TWEEN.Tween( this.reticle_object.scale ).to( {
                x: 3,
                y: 3,
                z: 3 }, 500 )
                .easing( TWEEN.Easing.Elastic.Out).start(); 
                tweenStart = this.clock.getElapsedTime();    
            }
            if( this.reticle_object.scale.x > 2.9){
                pow = false;
                console.log(pow);
            } 
            if(!pow){
